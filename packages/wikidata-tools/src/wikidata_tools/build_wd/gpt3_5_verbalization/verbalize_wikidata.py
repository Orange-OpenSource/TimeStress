...
from wikidata_tools.build_wd.gpt3_5_verbalization.utils import (
    parallel_verbalize,
    Entity,
    KnowledgeTriple,
    Literal,
...
from wikidata_tools.build_wd.utils.wd import db, get_info_wikidata, get_value
...
from wikidata_tools.build_wd.utils.general import run_in_process
...
RESUME = True
WHAT_TO_VERBALIZE = "old_wikidata"  # Can take two values ['old_wikidata', 'wfd']
SAVE_FOLDER = osp.join(STORAGE_FOLDER, "chatgpt_%s_verbalization" % WHAT_TO_VERBALIZE)
assert WHAT_TO_VERBALIZE in ("old_wikidata", "wfd")
...
class PropertyInfoRetriever:
    ...
    def retrieve_example(self, prop: Property, version="old"):
        ...
        sub_obj = db[adapt_name(version)].find_one(
            {"_id": prop.id},
            {
