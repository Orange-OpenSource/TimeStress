...
########## IMPORTANT ###########
# This file is not working anymore due to the update of the OpenAI python package
# update the calls to OpenAI models to work again
...
class Property(Entity):
    def __init__(
        ...
        label: str,
        description: str,
        triple_example: KnowledgeTriple = None,
    ) -> None:
...
class KnowledgeTriple:
    ...
    def __init__(
        self, subject: Entity, relation: Property, object: Union[Entity, Literal]
    ) -> None:
    ...
    def __eq__(self, __value: object) -> bool:
        ...
        if KnowledgeTriple.HASH_LEVEL != "GROUP":
            return (
                self.subject.id == __value.subject.id
                and self.relation.id == __value.relation.id
                and self.object.label == __value.object.label
            )
        else:
            return (
                self.subject.id == __value.subject.id
...
class ChatGPTVerbalizer:
    ...
    def verbalize(self, triple: KnowledgeTriple, use_support: bool = False):
        ...
        if triple.relation.triple_example is not None:
            ...
            prompt += (
                ' Finally, here is an example where the relation "%s" is employed : (%s, %s, %s).'
                % (example.relation.label, *example.get_labels())
            )
        try:
...
def analyze_verbalizations(
    triple: KnowledgeTriple, verbalizations: dict, chatgpt_options: dict
) -> dict:
    verbalizations_, total_tokens, chatgpt_response = (
        verbalizations.get("verbs", None),
        verbalizations.get("total_tokens", None),
        verbalizations.get("chatgpt_response", None),
    )
    verbs = []
    ...
    for v in verbalizations_:
        ...
        if ends_with_object:
            fill_in_the_blank = re.sub(
                rf"{re.escape(object_label)}[\.]?$", r"____", v, flags=re.IGNORECASE
            )
        else:
...
def parallel_verbalize(
    triples_options_chatgpt: Iterator[tuple[KnowledgeTriple, dict]],
    save_folder: str,
    n_threads=8,
    n_limit=None,
    n_triples=None,
    ...
    if resume:
        ...
        main_prompt, support_prompt, max_tokens = (
            MAIN_PROMPT,
            SUPPORT_PROMPT,
            MAX_TOKENS,
        )
        try:
            ...
            build_object = lambda x: (
                Entity(**x["object"])
                if len(x["object"]) == 3
                else Literal(**x["object"])
            )
            seen_triples = set(
                KnowledgeTriple(
                    Entity(**x["subject"]), Property(**x["relation"]), build_object(x)
                )
                for x in seen_triples
            )
        except FileNotFoundError:
            print(
                "Could not resume verbalization :: config.json and/or verbalizations.json not found!"
            )
    ...
    for i, d in enumerate(
        pool.imap(_thread_map, _generator(seen_triples), chunksize=1),
        start=len(seen_triples),
    ):
        if d is None:
            continue
