...
from wikidata_tools.build_wd.utils.ku import correct_time
from wikidata_tools.build_wd.utils.wd import db, sim_dict
...
parser.add_argument(
    "--old_wd_coll_name",
    help="The Old Wikidata collection name.",
    required=True,
)
parser.add_argument(
    "--new_wd_coll_name",
    help="The new Wikidata collection name.",
    required=True,
)
parser.add_argument(
    "--out_coll_name",
    help="The difference (output) collection name.",
    required=True,
)
parser.add_argument(
...
def new_in_wikidata_categorize(entity: dict):
    ...
    for i in range(len(datafield)):
        try:
            times.append(
                np.datetime64(correct_time(datafield[i]["value"]["time"]), "D")
            )
        except ValueError:
            continue
    if len(times) == 0:
    ...
    def closest_to(self, dicts: list[DictWithIgnore], to_ignore_var: list[str]) -> int:
        ...
        equals = [
            (
                i,
                sim_dict(
                    get_value(self.ig, to_ignore_var),
                    get_value(dicts[i].ig, to_ignore_var),
                    depth_penalty=False,
                ),
            )
            for i in equals
...
old_coll = db[args.old_wd_coll_name]
new_coll = db[args.new_wd_coll_name]

diff_coll = db[args.out_coll_name]
diff_coll.drop()


visited_new = set()
physically_new_entities = []
coll_physically_new_entities = db[
...
for i, old_ent_dict in enumerate(old_coll.find(), 1):
    ...
    if all(
        snak["diff_flag"] == "new" for snaks in diff["diff"].values() for snak in snaks
    ):
    ...
    if i % 1000 == 0:
        ...
        if len(physically_new_entities):
            coll_physically_new_entities.insert_many(
                [{"_id": x, "date_of_creation": y} for x, y in physically_new_entities],
                ordered=False,
            )
            physically_new_entities.clear()
        print(
            "[Phase 1/2 (Old)] %s/%s (%0.3f%%)"
            % (i, old_coll_size, i / old_coll_size * 100)
        )
        GlobalStat.save(
            osp.join(
                STORAGE_FOLDER,
                "resources/script_stats/compute_diff__%s__%s.json"
...
if len(physically_new_entities):
    coll_physically_new_entities.insert_many(
        [{"_id": x} for x in physically_new_entities], ordered=False
    )
    physically_new_entities.clear()
...
for i, new_ent_dict in enumerate(new_coll.find(), 1):
    ...
    if i % 1000 == 0:
        ...
        if len(physically_new_entities):
            ...
            physically_new_entities.clear()
        print(
            "[Phase 2/2 (New)] %s/%s (%0.3f%%)"
            % (i, new_coll_size, i / new_coll_size * 100)
        )
        GlobalStat.save(
            osp.join(
                STORAGE_FOLDER,
