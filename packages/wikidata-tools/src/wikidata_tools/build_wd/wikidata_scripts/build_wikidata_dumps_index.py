...
def process_wikidata_dump(d: str):
    ...
    files = [
        x for x in re.findall(r'<a href="(.+?)">.+?</a>', html) if re.match(pattern, x)
    ]
...
if __name__ == "__main__":
    try:
        ...
        identifiers = search_items(
            'creator:"Wikidata editors" AND mediatype:"web" AND collection:"wikicollections" AND title:"Wikidata entity dumps (JSON and TTL) of all Wikibase entries for Wikidata generated on"'
        )
