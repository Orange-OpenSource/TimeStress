...
from wikidata_tools.build_wd.config import REWIND_N_MONTHS


# WIKI_PAGEVIEWS_DUMPS_URL = "https://dumps.wikimedia.org/other/pageview_complete/monthly/"


def main(version: str, time_: str, n_rewind: int):
    REWIND_N_MONTHS = n_rewind
    if time_ is not None:
        WIKIPEDIA_DATE = time_
    else:
    ...
    CHUNK_SIZE = 100 * 2**20
    if time_ is not None:
        WIKIPEDIA_MONTH_CONSULT_MDB_COLLECTION_NAME = (
            "%s_wikipedia_month_consultation" % time_
        )
    else:
        WIKIPEDIA_MONTH_CONSULT_MDB_COLLECTION_NAME = (
            "%s_wikipedia_month_consultation" % time_
    ...
    collection_tmp.drop()
    collection_tmp.create_index([('title', pymongo.ASCENDING), ('year_month', pymongo.ASCENDING)])
    print("OK.")
    ...
    for d in list_dates:
        if not check_url(get_url(*d)):
            print(
                f"WARNING : pageviews is not available for {d[0]}-{d[1]}. Skipping this month !"
            )
        else:
    ...
    # Download dumps
    commands = [
        ("bash %s -c " % osp.join(osp.dirname(__file__), "../utils/wget_lock.sh")) + x
        for x in urls_to_download
    ]
    ...
    def target(command: str):
        while True:
            state = subprocess.run(
                ("cd %s && " % TEMPORARY_DIRECTORY) + command,
                shell=True,
                stdin=None,
                stdout=sys.stdout,
                stderr=sys.stderr,
    ...
    for file in list_files:
        ...
        date2file[year_month] = indexed_bzip2.open(
            os.path.join(TEMPORARY_DIRECTORY, file), parallelization=8
        )
    ...
    def process_line(line: bytes, last_title: str = None, last_id: int = None):
        ...
        if len(splits) != 6:
            ret = (
                ProcessState.FROM_WIKI_BUT_BAD_ID
                if splits[0] == WIKIPEDIA_TO_FETCH
                else ProcessState.NOT_FROM_WIKI
            )
        ...
        if splits[0] != WIKIPEDIA_TO_FETCH:
            return ProcessState.NOT_FROM_WIKI, (
                project,
                title,
                page_id,
                month_consultation,
        ...
        if page_id == b"null" and title == last_title:
            page_id = last_id
        if page_id == b"null":
            return ProcessState.FROM_WIKI_BUT_BAD_ID, (
                project,
                title,
                page_id,
                month_consultation,
    ...
    while True:
        ...
        with timeit_read_chunk:
            chunks = {
                k: v.read(CHUNK_SIZE // len(date2file)) + v.readline()[:-1]
                for k, v in date2file.items()
            }
        ...
        for year_month in chunks.keys():
            ...
            #     "Continue until new page id {}".format(year_month)
            # ):
        ...
        max_title = max(
            [x for x in date2last.values() if x[0] is not None], key=lambda x: x[0]
        )[0]
        ...
        #                 if success != ProcessState.OK:
        #                     if (
        #                         success == ProcessState.NOT_FROM_WIKI
        #                         and res[0] > WIKIPEDIA_TO_FETCH
        #                     ):
        #                         break
        #                     continue
        #                 _, last_title, last_id, month_consultation = res
            ...
            for year_month in chunks.keys():
                for title, id_, month_consultation in chunks[year_month]:
                    ...
                    inserts.append({
                            ...
                            "n": month_consultation,
                            "page_id": id_.decode()
                    })
                    # if title in inserts:
                    #     inserts[title]["month_consults"].append(
                    #         {"year_month": year_month, "n": month_consultation}
                    #     )
                    # else:
                    #     inserts[title] = {
                    #         "_id": {'title': title, "year_month": year_month},
                    #         "page_id": id_.decode(),
                    #         "n": month_consultation
                    #     }

        with timeit_mongo_insert:
            collection_tmp.insert_many(inserts, ordered=False)
        ...
        for year_month, file in date2file.items():
            ...
            print(
                "Progress File {} : {}/{}GB".format(
                    year_month,
                    round(pos / 2**30, 2),
                    round(date2size[year_month] / 2**30, 2),
                )
            )
        print(
            "\nTotal progress : {}/{}GB".format(
                round(sum(date2last_ckp.values()) / 2**30, 2),
                round(total_size / 2**30, 2),
            )
        )
        print(
            "Process speed : {}MB/s\n".format(
                round(processed_all / (t2 - t1) / 2**20, 2)
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--time",
        help="Wikidata instance for which to collect views stats (must be a date in format YYYYMMDD)",
        required=False,
        default=None,
    )
    # parser.add_argument(
    ...
    t1 = time.time()
    main(args.version, args.time, REWIND_N_MONTHS)
