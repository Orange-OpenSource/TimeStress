...
from wikidata_tools.build_wd.utils.wd import (
    ...
    get_info_wikidata,
    get_direct_indirect_types,
)
from wikidata_tools.build_wd.utils.ku import (
    get_start_end_time,
    get_point_in_time,
)
...
class IrrelevantWikiEntDetector:
    ...
    def classify(self, ent_id: str, types: list[str]):
        if ent_id in self.relevant_entities:
            ...
            return True
        if types is None:
            types = get_info_wikidata(
                ent_id, version=self.version, return_types_ids=True
            ).get("types_ids", [])
        if len(types) == 0:
            return False
        for t in types:
            if t in self.irrelevant:
                ...
                return True
        unk_types = [
            t for t in types if t not in self.irrelevant and t not in self.relevant
        ]
...
class MetaPropertiesDetector:
    ...
    def hard_check_version(self, prop_id: str):
        return (
            MetaPropertiesDetector.META_PROPERTIES_ENTITY_FILTER
            in get_direct_indirect_types(prop_id, version=self.version, depth=4)
        )
...
def to_dt64(time_: str):
    return np.datetime64(
        datetime.strptime(time_, "%Y%m%d"), "D"
    )

old_timestamp = to_dt64('20210104')
new_timestamp = to_dt64('20230227')
...
class StatementTransformer:
    ...
    def __init__(self, version: str) -> None:
        ...
        self.reference_timestamp = (
            new_timestamp if version == "new" else old_timestamp if version == 'old' else to_dt64(version)
        )
        self.single_valued_properties = get_single_valued_properties(version, return_separator=True)
        self.temporal_single_value_properties = [
            k for k, v in self.single_valued_properties.items() if "P585" in v
        ]
    ...
    def is_deprecated_end_start_time(self, statement: dict):
        ...
        res = (
            ...
            and start_time > self.reference_timestamp
            or end_time is not None
            and end_time < self.reference_timestamp
        )
    ...
    def collect_validity_indicators(self, prop_id: str, statement: dict):
        ...
        # ]:
        #     for date, timestamp in [
        #         ("old", old_wikidata_timestamp),
        #         ("new", new_wikidata_timestamp),
        #     ]:
    ...
    def is_non_unique(self, statements: list[dict]) -> bool:
        values = [
            default_to_frozen(statement["mainsnak"]["datavalue"])
...
def main(version: str | None, collection_in: str, collection_out: str, time_: str):
    coll_name = "wikidata_%s_json" % version if version is not None else collection_in
    if version is None and time_ is not None:
        version = time_
    ...
    if collection_out is None:
        collection_out = "wikidata_%s_prep" % version
    coll = db[collection_out]
    coll.drop()
    coll.create_index([("label_upper", 1)])
    coll.create_index([("aliases", 1)])
    ...
    for i, ent_dict in enumerate(cursor, 1):
        ...
        if irrelevant_wikient_det.classify(
            ent_dict["_id"], irrelevant_wikient_det.get_types(ent_dict)
        ):
            GlobalStat.n_irrelevant_entities += 1
        ...
        for prop_id, statements in list(claims.items()):
            ...
            statements[:] = [
                statement
                for statement in statements
                if not has_special_value(statement)
            ]
            ...
            statements[:] = [
                statement
                for statement in statements
                if not restricted_triplet_det.is_restricted(statement)
            ]
            if not group_empty and len(statements) == 0:
            ...
            for statement in statements:
                ...
                # GlobalStat.pit_status_count[
                #     statement["validity"].get("pit_status", "none")
                # ] += 1
        ...
        if i % 1000 == 0:
            ...
            GlobalStat.save(
                osp.join(
                    STORAGE_FOLDER,
                    "resources/script_stats/preprocess_dump_%s.json" % collection_out,
                )
            )
    ...
    with open(
        osp.join(
            STORAGE_FOLDER,
            "resources/property_lists/post_preproc_properties_%s.json" % collection_out,
        ),
        "w",
    ) as f:
    ...
    with open(
        osp.join(
            STORAGE_FOLDER,
            "resources/property_lists/restrictive_qualifiers_%s.json" % collection_out,
        ),
        "w",
    ) as f:
    ...
    with open(
        osp.join(
            STORAGE_FOLDER,
            "resources/property_lists/meta_properties_%s.json" % collection_out,
        ),
        "w",
    ) as f:
        json.dump(list(GlobalStat.meta_relations_count.keys()), f)

    # with open(osp.join(STORAGE_FOLDER, '../resources/knn/prop_obj_counts_%s.json' % collection_out), 'w') as f:
    #     json.dump(list(property_object_counts.items()), f)
    os.makedirs(osp.join(STORAGE_FOLDER, "resources/entity_lists"), exist_ok=True)
    with open(
        osp.join(
            STORAGE_FOLDER,
            "resources/entity_lists/irrelevant_entities_%s.json" % collection_out,
        ),
        "w",
    ) as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--collection_in",
        help="The name of the Wikidata collection to preprocess given date (input collection)",
        required=False,
        default=None,
    )
    parser.add_argument(
        "--collection_out",
        help="Name of the preprocessed Wikidata collection (output collection)",
    )
    parser.add_argument(
        "--time",
        help="Timestamp of the Wikidata dump (Format : YYYYMMDD)",
        default=None,
    )
    ...
    main(
        ...
        collection_in=args.collection_in,
