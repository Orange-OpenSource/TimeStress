...
def process_io(vars):
    ...
    for i, chunk in enumerate(
        db[COLLECTION_NAME].find_raw_batches(batch_size=N_DOCUMENT_CHUNK)
    ):
...
def process_chunk(vars):
    ...
    while vars["io_alive"].value or vars["queue_io"].qsize():
        ...
        ent2types = {}
        consults = list(
            db["%s_wikipedia_month_consultation" % version].find(
                {"_id": {"$in": list(pointers.keys())}},
                {"_id": 1, "month_consults": 1},
                batch_size=N_DOCUMENT_CHUNK,
            )
        )
        for d in consults:
            ...
            if "P31" not in diff:
                ...
                for statement in [
                    s for s in diff["P31"] if s["diff_flag"] != anti_version
                ]:
        ...
        for ent in chunk_full.values():
            ...
            for prop, statements in diff.items():
                for statement in [
                    s for s in statements if s["diff_flag"] != anti_version
                ]:
                    if len(ent_types):
                        for t in ent_types:
    ...
    while True:
        ...
        print(
            "Speed : {} entities/sec".format(
                round(position.value / (time.time() - start_time), 2)
            )
        )
...
def main(version: str, time: str):
    ...
    new_date = np.datetime64(WIKIDATA_DATE[:4] + "-" + WIKIDATA_DATE[4:6])
    ...
    p_stat = mp.Process(
        ...
        args=(
            "Wikidata %s - Acquiring importance stats (entity types + property)"
            % version.upper(),
    ...
    db["%s_entity_type_property_importance" % version].insert_many(
    ...
    with open(
        osp.join(
            ...
            osp.splitext(osp.basename(__file__))[0] + "_stats.json",
        ),
        "w",
    ) as f:
        json.dump(stats, f)
    ...
    db["%s_entities_types_importance" % version].insert_many(
