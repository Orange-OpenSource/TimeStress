...
parser.add_argument(
    "--version",
    help="Wikidata version to process",
    type=str,
    required=False,
    choices=["old", "new"],
)
parser.add_argument(
    "--date",
    help="Wikidata version to process (select version using date)",
    type=str,
    required=False,
)
parser.add_argument(
    "--collection_name", help="MongoDB collection name", type=str, default=None
)
parser.add_argument(
    "--limit",
    type=int,
    help="limit push to mongodb to the n first bz2 chunks in the wikidata JSON dump (useful for testing purposes)",
    required=False,
    default=None,
)
parser.add_argument(
    "--n_cpu_bz2_decompression",
    help="Number of parallel processes that will be used for BZ2 decompression. Defaults to {}".format(
        N_CPU_BZ2_DECOMPRESSION
    ),
    type=int,
    default=N_CPU_BZ2_DECOMPRESSION,
)
parser.add_argument(
    "--n_cpu_process_chunk",
    help="Number of parallel processes that will be used to process chunks. Defaults to {}".format(
        N_CPU_PROCESS_CHUNK
    ),
    type=int,
    default=N_CPU_PROCESS_CHUNK,
)
parser.add_argument(
    "--n_cpu_mongodb_insert",
    help="Number of parallel processes that will be used to push data to MongoDB. Defaults to {}".format(
        N_CPU_MONGODB_INSERT
    ),
    type=int,
    default=N_CPU_MONGODB_INSERT,
)
parser.add_argument(
    "--chunk_size",
    help="Size of uncompressed chunks in bytes. Defaults to {} which means {}MB".format(
        CHUNK_SIZE, round(CHUNK_SIZE / 2**20, 4)
    ),
    type=int,
    default=CHUNK_SIZE,
)
parser.add_argument(
    "--truthy",
    help="If specified, get the truthy version of Wikidata, i.e., keep only statements with preferred rank when provided and remove deprecated statements. If unspecified, remove only deprecated statements",
    action="store_true",
)
parser.add_argument(
    "--stat_only",
    help="Do not create any collection and do not push data to MongoDB. The only output of this script is the stats computed from the given Wikidata dump. If activated, there will be a substantial gain execution time",
    action="store_true",
)
...
def process_property(entity: dict, stats: dict):
    ...
    for k, v in entity["claims"].items():
        ...
        for d in v:
            stats["datatype_n_%s" % d["mainsnak"]["datatype"]] = (
                stats.get("datatype_n_%s" % d["mainsnak"]["datatype"], 0) + 1
            )
    return True
...
def process_entity(entity: dict, truthy: bool, stats: dict):
    ...
    for k, v in entity["claims"].items():
        ...
        for i, d in enumerate(v):
            try:
                ...
                if d["mainsnak"]["datatype"] in BANNED_DATATYPES:
                    stats["n_triples_banned_types"] = (
                        stats.get("n_triples_banned_types", 0) + 1
                    )
                    continue
                elif d["rank"] == "deprecated":
                    stats["n_triples_deprecated"] = (
                        stats.get("n_triples_deprecated", 0) + 1
                    )
                    continue
                to_keep.append(i)
                ranks.append(d["rank"])
        ...
        if truthy:
            if len(v) and any(x == "preferred" for x in ranks):
                to_keep = [
                    to_keep[j] for j in range(len(to_keep)) if ranks[j] == "preferred"
                ]
        if len(to_keep):
            ...
            for i in to_keep:
                ...
                if "qualifiers" in to_filter:
                    ...
                    to_filter["qualifiers"] = {
                        k: [x for x in v] for k, v in to_filter["qualifiers"].items()
                    }

                    # Remove empty qualifiers
                    to_filter["qualifiers"] = {
                        k: v for k, v in to_filter["qualifiers"].items() if len(v)
                    }
    ...
    _tmp = entity["claims"]
    entity["claims"] = {
        k: [_tmp[k][i] for i in to_keep] for k, to_keep in statements_to_keep
    }
...
def process_io(
    ...
    count_chunks = 0
    while True:
        if limit > 0 and count_chunks >= limit:
            break
...
def process_stats(
    queue_io: mp.Queue, queue_mongo: mp.Queue, file_position: float, file_size: float
):
...
if __name__ == "__main__":
    ...
    if coll_name is None:
        if date is not None:
            coll_name = "wikidata_%s_json" % date
        else:
    ...
    prefix = version if date is None else date
    path = osp.join(STORAGE_FOLDER, "%s_wikidata.json.bz2" % prefix)
    ...
    p_io = mp.Process(
        ...
        args=(
            ...
            path,
            file_position,
            args.n_cpu_bz2_decompression,
            args.n_cpu_process_chunk,
            args.chunk_size,
            limit,
        ),
    )
    p_c = [
        mp.Process(
            ...
            args=(
                io_q,
                queue_mongo,
                n_process_chunk_alive,
                truthy,
                stats_queue,
                args.stat_only,
            ),
        )
        for _ in range(args.n_cpu_process_chunk)
    ]
    p_stats = mp.Process(
        target=process_stats,
        args=(io_q, queue_mongo, file_position, os.path.getsize(path)),
    )
    if not args.stat_only:
        p_mongo = [
            mp.Process(
                target=process_mongo,
                args=(queue_mongo, n_process_chunk_alive, coll_name),
            )
            for _ in range(args.n_cpu_mongodb_insert)
        ]
    ...
    with open(
        osp.join(
            STORAGE_FOLDER,
            "resources/script_stats/process_json_dump_%s.json" % coll_name,
        ),
        "w",
    ) as f:
