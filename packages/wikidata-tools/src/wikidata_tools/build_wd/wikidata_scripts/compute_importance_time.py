...
def month_consults2importance(
    month_consults: list[dict], list_yearmonths: list[str]
) -> float:
...
def process_io(vars):
    ...
    for i, chunk in enumerate(
        db["wikidata__%s__PREPROCESSED" % time].find_raw_batches(
            batch_size=N_DOCUMENT_CHUNK
        )
    ):
...
def process_chunk(vars):
    ...
    while vars["io_alive"].value or vars["queue_io"].qsize():
        ...
        chunk = list(
            db["wikidata__%s__ALMOST_RAW" % time].find(
                {"_id": {"$in": chunk}, "enwiki_title": {"$exists": 1}},
                {"enwiki_title": 1},
                batch_size=N_DOCUMENT_CHUNK,
            )
        )
        ...
        ent2types = {}
        consults_raw = list(
            db["%s_wikipedia_month_consultation" % time].find(
                {"title": {"$in": list(pointers.keys())}},
                {"title" : 1, "year_month": 1, "n": 1},
                batch_size=N_DOCUMENT_CHUNK,
            )
        )
        # Adapt consults to format as a list of
        # {"_id" : title, "month_consults" : year2consultation_dict}
        consults = {}
        for c in consults_raw:
            title = c.pop('title')
            if (existing := consults.get(title)):
                ymdict = existing['month_consults']
                ym = c.pop('year_month')
                if ym not in ymdict:
                    ymdict[ym] = c.pop('n') 
            else:
                existing = {'_id' : title, 'month_consults' : {c.pop('year_month') : c.pop('n')}}
                consults[title] = existing
        ...
        for d in consults:
            ...
            if "P31" not in claims:
                ...
                for statement in claims["P31"]:
                    id_ = statement["mainsnak"]["datavalue"]["value"]["id"]
    ...
    vars["queue_info"].put(
        {
            "not_found_entities": not_found_entities,  # Entities that were not found using month consult database
            "entities_importance": entities_importance,  # Entities that were found and their corresponding importance
            "entities_types_importance_stats": default_to_regular(
                entities_types_importance_stats
            ),  # Importance of each entity type
            "entity_type_property_importance_stats": default_to_regular(
                entity_type_property_importance_stats
            ),  # Importance of properties in entity types
...
def process_stats(
    phase_name: str, position: int, n_documents: int, queues_dict: dict[str, mp.Queue]
):
    ...
    while True:
        ...
        print(
            "Progress : {}/{} wikipedia entities processed".format(
                position.value, n_documents
            )
        )
        print(
            "Speed : {} entities/sec".format(
                round(position.value / (time_pack.time() - start_time), 2)
            )
        )
...
def main(time: str):
    ...
    new_date = np.datetime64(time[:4] + "-" + time[4:6])
    ...
    client = MongoClient(MONGO_URL)
    db = client[MONGODB_NAME]
    n_documents = db["wikidata__%s__PREPROCESSED" % time].count_documents({})
    ...
    ps_chunk = [
        mp.Process(target=process_chunk, args=(process_chunk_vars,))
    ...
    p_stat = mp.Process(
        target=process_stats,
        args=(
            "Wikidata %s - Acquiring importance stats (entity types + property)" % time,
            process_io_vars["position"],
            n_documents,
            queue_dict,
        ),
    )
    ...
    entities_types_importance_stats = {
        k: np.log(v["sum"] / v["n"] + 1)
        for k, v in entities_types_importance_stats.items()
    }
    ...
    for i in range(len(entities_importance)):
        entities_importance[i]["importance"] = np.log(
            entities_importance[i]["importance"] + 1
        )
    ...
    ent_prop_imp = [
        {"_id": {"ent_type": k, "prop": k2}, "imp": v2}
        for k, v in entity_type_property_importance_stats.items()
        for k2, v2 in v.items()
    ]
    db["%s_entity_type_property_importance" % time].drop()
    db["%s_entity_type_property_importance" % time].insert_many(
        ent_prop_imp, ordered=False
    )
    del ent_prop_imp

    # Saving stats
    stats = {
        "not_found_entities": len(not_found_entities),
        "n_entities": n_documents,
        "found_entities": len(entities_importance),
    }
    # assert stats['not_found_entities'] + stats['found_entities'] == stats['n_entities']
    with open(
        osp.join(
            STORAGE_FOLDER,
            "resources/script_stats",
            osp.splitext(osp.basename(__file__))[0] + "_%s_stats.json" % time,
        ),
        "w",
    ) as f:
        json.dump(stats, f)
    ...
    ################## PHASE 2 ##################
    print(
        "Computing entity importance for not-found entities (this can take sometime)..."
    )
    ...
    def get_ent_imp(ent_id):
        try:
            ...
            if len(types):
                ...
                if len(types) == 0:
                    ...
                    return {
                        "ent_imp": med_entities_importance,
                        "types": types,
                        "imp_found_using": "overall_median_imp",
                    }
        except KeyError:
            types = [None]
        if len(types) == 0:
        ...
        if ent_id not in entities_importance:
            ...
            return {
                "ent_imp": ent_imp,
                "types": types,
                "imp_found_using": imp_found_using,
            }
        return {
            "ent_imp": entities_importance[ent_id],
            "types": types,
            "imp_found_using": "ent_imp_dictionary",
        }
    ...
    entities_types_importance_stats = [
        {"_id": k, "importance": v} for k, v in entities_types_importance_stats.items()
    ]
    db["%s_entities_types_importance" % time].drop()
    db["%s_entities_types_importance" % time].insert_many(
        entities_types_importance_stats, ordered=False
    )

    print("Push entities type...")
    db["%s_entities_types" % time].drop()
    entities_types = [{"_id": k, "types": v} for k, v in entities_types.items()]
    db["%s_entities_types" % time].insert_many(entities_types)

    print("Push entities importance (this can take sometime)...")
    db["%s_entities_importance" % time].drop()
    db["%s_entities_importance" % time].create_index([("ent_imp", -1)])
    db["%s_entities_importance" % time].insert_many(entities_importance_final)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--time",
        help="Timestamp at which the importance is computed. Must be a date in YYYYMMDD format",
        required=True,
    )
    args = parser.parse_args()
    t1 = time_pack.time()
    main(args.time)
    print("Execution time = %ssec" % (time_pack.time() - t1))
