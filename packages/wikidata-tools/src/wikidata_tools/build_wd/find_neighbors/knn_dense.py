...
import numpy as np
from _old_codebase.find_neighbors.config import EMBEDDINGS_FOLDER
from _old_codebase.find_neighbors.scripts.generate_entity_label_embedding import (
    main as main_generate_label_embedding,
)
from _old_codebase.find_neighbors.scripts.index_with_nmslib import (
...
def _check_if_encoding_done():
    return osp.exists(
        osp.join(DEFAULT_KNN_FILES_PATH, "encoded_sentences_0.h5")
    ) and osp.exists(osp.join(DEFAULT_KNN_FILES_PATH, "encoded_sentences_0.jsonl"))


def _check_if_nmslib_index_exists():
    return osp.exists(osp.join(DEFAULT_KNN_FILES_PATH, "entities.txt")) and osp.exists(
        osp.join(DEFAULT_KNN_FILES_PATH, "nmslib_embeddings.idx")
    )


class KNearestNeighbors:
    def __init__(
        self, model_name="EleutherAI/gpt-neo-2.7B", version=["old", "new"]
    ) -> None:
    ...
    def setup(self, reset=False):
        if reset or not _check_if_encoding_done():
            ...
            print(
                "IMPORTANT : If the progress bar is stucked, it means that no GPU were available when the script was launched or that something went wrong."
            )
            print("You can restart the program if this happens. Progress is saved.")
            for v in self.version:
                main_generate_label_embedding(
                    "entity_labels", self.model_name, version=v
                )
    ...
    def load_index(self):
        if not _check_if_nmslib_index_exists():
            raise Exception(
                "Error : Index does not exist. Call the setup() function to create it."
            )
        ...
        for x in os.listdir(DEFAULT_KNN_FILES_PATH):
            if m := re.match(r"^encoded_sentences_([0-9]+)\.h5$", x):
                ...
                with open(
                    osp.join(
                        DEFAULT_KNN_FILES_PATH,
                        "encoded_sentences_%s.jsonl" % m.group(1),
                    )
                ) as f:
                    self.l_id2int.append(
                        {json.loads(x)["_id"]: i for i, x in enumerate(f.readlines())}
                    )
