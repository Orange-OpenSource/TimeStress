...
def main(subfolder_path: str):
    ...
    for x in os.listdir(subfolder_path):
        if m := re.match(r"^encoded_sentences_([0-9]+)\.h5$", x):
            print("Reading %s..." % x)
            with open(
                osp.join(subfolder_path, "encoded_sentences_%s.jsonl" % m.group(1))
            ) as f:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Script for create a NMSlib index from entity labels embeddings."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Folder that contains the entity label embeddings",
    )
