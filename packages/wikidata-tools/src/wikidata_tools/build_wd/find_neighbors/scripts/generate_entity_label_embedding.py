...
def gpu_worker(
    subfolder_path, gpu_id, model_name: str, task_queue: Queue, start_index: int
):
    ...
    with h5py.File(
        osp.join(subfolder_path, f"encoded_sentences_{gpu_id}.h5"), "a"
    ) as f:
        ...
        while True:
            ...
            embeddings = (
                all_hidden_states[range(len(task)), last_token_positions].cpu().numpy()
            )
...
def label_generator(version, batch_size, seen_entities: set):
    ...
    for x in tqdm.tqdm(
        db["wikidata_%s_prep" % version].find({"label": {"$exists": 1}}, {"label": 1}),
        total=n,
    ):
...
def main(
    ...
    if resume == "no":
        ...
        for file in os.listdir(subfolder_path):
            if m := re.match(r"^encoded_sentences_([0-9]+)\.jsonl$", file):
                ...
                assert osp.exists(
                    osp.join(subfolder_path, "encoded_sentences_%s.h5" % m.group(1))
                )
    # try:
    ...
    ps = [
        Process(
            target=gpu_worker,
            args=(subfolder_path, i, model_name, task_queue, int2start[i]),
        )
        for i in range(num_gpus)
        if gpu_available_memory[i] > mm_gpu_filter
    ]
    for p in ps:
        p.start()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Script for encoding sentences with Large Language Models from huggingface."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=RESUME,
        help='Attempt to resume the encoding. Can take three values : "no" do not attempt a resume, "attempt" will attempt a resume (DATA VALIDITY NOT GUARANTEED). Defaults to "%s"'
        % RESUME,
        choices=["no", "yes"],
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=VERSION,
        help='From which wikidata version to retrieve labels. Defaults to "%s"'
        % VERSION,
        choices=["old", "new"],
    )
    parser.add_argument(
        "--mm_gpu_filter",
        type=int,
        default=MM_GPU_FILTER,
        help="Do not use GPUs that have less memory than this argument (in MBs). Defaults to %s"
        % MM_GPU_FILTER,
    )
