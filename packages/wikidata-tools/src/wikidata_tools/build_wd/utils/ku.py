...
# from wikidata_tools.build_wd.config import OLD_WIKIDATA_DATE, NEW_WIKIDATA_DATE
# from datetime import datetime
import numpy as np

# Precision is the priority with these tests, not recall

new_dateproperty2text = dict(
    P571=dict(name="inception"),
...
property2reduce = {"oldest": lambda x: min(x)}

# old_wikidata_timestamp = np.datetime64(
#     datetime.strptime(OLD_WIKIDATA_DATE, "%Y%m%d"), "D"
# )
# new_wikidata_timestamp = np.datetime64(
#     datetime.strptime(NEW_WIKIDATA_DATE, "%Y%m%d"), "D"
# )
...
class FeatureDiff:
    ...
    def __init__(
        ...
        prop_id: str = None,
        is_old: bool = None,
        ent_is_ph_new: bool = None,
        obj_is_ph_new: bool = None,
    ) -> None:
        ...
        if snak is not None:
            ...
            self.start_time, self.end_time, self.start_time_old, self.end_time_old = (
                get_start_end_time_old_and_new(snak)
            )
            ...
            self.value, self.relation_type, self.start_time, self.end_time, self.point_in_time, self.start_time_old, self.end_time_old, self.point_in_time_old, self.diff_flag = [None]*9
...
def classify_algorithm_lite(features: Union[Feature, list[Feature]], reference_timestamp : np.datetime64):
    # WARNING : No guarantee for version='new'
    if reference_timestamp is None:
        reference_timestamp = new_wikidata_timestamp
    ...
    for feature in features:
        ...
        if feature.end_time is not None and feature.end_time < reference_timestamp:
            ...
            feature.start_time is not None and feature.start_time > reference_timestamp
        ):
            decision = "ignore"
...
def classify_algorithm(features: list[FeatureDiff], old_wikidata_timestamp : np.datetime64, new_wikidata_timestamp : np.datetime64):
    # features = sorted(features, key = lambda x : x.diff_flag)
    decisions = []
    paths = []
    for feature in features:
        decision = "unk"
        path = "unk"
        if (
            feature.start_time is not None
            and feature.end_time is not None
            and feature.start_time > feature.end_time
        ):
            decision = "unk"
            ...
            if count["old"] == 1 and count["new"] == 1 and count["both"] == 0:
                if feature.diff_flag == "new":
                    if (
                        feature.point_in_time is not None
                        and new_wikidata_timestamp
                        > feature.point_in_time
                        > old_wikidata_timestamp
                    ):
                        path = (
                            "temp_rel",
                            "1_old_1_new",
                        ...
                        if (
                            feature.end_time is None
                            or feature.end_time is not None
                            and feature.end_time > new_wikidata_timestamp
                        ):
                            decision = "learn"
                            path = (
                                "temp_rel",
            ...
            if (
                feature.diff_flag == "new"
                and feature.point_in_time is not None
                and new_wikidata_timestamp
                > feature.point_in_time
                > old_wikidata_timestamp
            ):
                ...
                feature.end_time is not None
                and feature.end_time < old_wikidata_timestamp
            ):
                path = "other_rel", "end < old", "ignore"
                decision = "ignore"
            elif (
                feature.end_time is None
                and feature.start_time is not None
                and feature.start_time < old_wikidata_timestamp
            ):
                ...
                feature.end_time is None
                and feature.start_time is not None
                and old_wikidata_timestamp < feature.start_time < new_wikidata_timestamp
            ):
                path = "other_rel", "end_none, start_between", "learn"
                decision = "learn"
            elif (
                feature.start_time is not None
                and feature.start_time > new_wikidata_timestamp
            ):
                ...
                feature.start_time is not None
                and feature.end_time is not None
                and new_wikidata_timestamp > feature.start_time > old_wikidata_timestamp
                and new_wikidata_timestamp > feature.end_time > old_wikidata_timestamp
            ):
                path = (
                    "other_rel",
                    "old < start < new, old < end < new",
                    "ignore_between",
                )
                decision = "ignore"
            elif (
                ...
                and feature.end_time is not None
                and feature.start_time < old_wikidata_timestamp
                and old_wikidata_timestamp < feature.end_time < new_wikidata_timestamp
            ):
                ...
                feature.diff_flag == "old"
                and feature.end_time_old is not None
                and feature.end_time_old < old_wikidata_timestamp
            ):
    ...
    for i, dec in enumerate(decisions):
        ...
        if (
            dec == "unk"
            and feature.diff_flag == "old"
            and feature.relation_type in ["func", "temp"]
            and len(features) == 2
            and decisions[(i + 1) % 2] == "learn"
        ):
...
def classify_algorithm_full(features: list[FeatureDiff], old_wikidata_timestamp : np.datetime64, new_wikidata_timestamp : np.datetime64):
    assert all(
        x is not None
        for feature in features
        for x in [feature.__getattribute__(y) for y in FeatureDiff.must_attributes]
    ), (
        " and ".join(FeatureDiff.must_attributes)
        + " must be set for all features to be able to call classify_algorithm_full"
    )
    ...
    if prop_id in ["P570", "P4602"]:
        ...
        if (
            len(features) == 1
            and features[0].diff_flag == "new"
            and t is not None
            and new_wikidata_timestamp > t > old_wikidata_timestamp
        ):
            return ["learn"], ["date_of_death/burial", "learn"]
        else:
            return ["unk"] * len(features), [("date_of_death/burial", "unk")] * len(
                features
            )

    return classify_algorithm(features, old_wikidata_timestamp, new_wikidata_timestamp)
